...
struct AST_Node* genVarAST()
{
	...
	if (currentToken->tokenType == TT_VAR)
	{
		...
		{
			node = mkastnode_ident(TT_IDENT, 0, 0, NULL, NULL, newID_token(DT_INT, 0, 0, varName, 1), varName);
			scan_curToken();
			return node;
		}
		else if (currentToken->tokenType == TT_EQUALS)
		{
			node = mkastnode_ident(TT_IDENT, 0, 0, binexpr_int(0), NULL, newID_token(DT_INT, 1, 0, varName, 1), varName);
			return node;
		}
...
struct AST_Node* genIdentAST()
{
	...
	{
		node = mkastnode_ident(TT_IDENT, 0, 0, binexpr_int(0), NULL, newID_token(DT_INT, 1, 0, varName, 0), varName);
		return node;
	}
